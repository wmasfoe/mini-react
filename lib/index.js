...
function workLoop(deadline) {
  ...
  // 修复没有调用 render 时，一直 requestIdleCallback
  if(wipRoot === null) {
    return console.error('not call LReact.render()')
  }
...
function updateFunctionComponent(fiber) {
  // fiber.type 是一个函数(函数组件)通过运行这个函数并传入 props 得到子组件
...
function commitDeletion(fiber, domParent) {
  if (fiber.dom) { // 如果 fiber 节点有 dom
    domParent.removeChild(fiber.dom)
  } else {
    // fiber 没有 dom 时，递归找到 fiber 下有 dom 的节点
